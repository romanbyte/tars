...
const jadeInheritance = require('yellfy-pug-inheritance');
var jadeInheritanceCache = {};
...
module.exports = () => {
    return gulp.task('html:compile-templates', () => {
        ...
        function makeGulpPipe(resolve, reject, inheritance) {
            return gulp.src(filesToCompile)
                ...
                .pipe(gulpIf(tars.templater.name === 'jade' && tars.options.watch.isActive, gulpFilter(function(file) {
                    const filepath = 'markup/pages/' + file.relative;

                    if (!tars.options.watch._changedTemplateFile || inheritance.checkDependency(filepath, tars.options.watch._changedTemplateFile)) {
                        gutil.log('Compiling: ' + gutil.colors.green.bold(filepath));
                        return true;
                    }
        ...
        if (tars.templater.name === 'jade') {
            return new Promise((resolve, reject) => {
                const options = {
                    changedFile: tars.options.watch._changedTemplateFile,
                    treeCache: jadeInheritanceCache,
                    jade: true
                };

                jadeInheritance.updateTree('./markup', options).then((inheritance) => {
                    jadeInheritanceCache = inheritance.tree;

                    return makeGulpPipe(resolve, reject, inheritance);
                });
            });
        }
